...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    ...
    case "fetchPhoto":
      handleFetchPhoto(request, sendResponse);
      return true;
...
// Refactored fetchPhoto handler
function handleFetchPhoto(request, sendResponse) {
  ...
  fetch(urlToFetch)
    .then((response) => {
      ...
      if (!contentType || !contentType.startsWith("image/")) {
        throw new Error(`Expected image, but received ${contentType || "no content type"}.`);
      }
    ...
    .then((blob) => {
      ...
      reader.onloadend = () => {
        chrome.storage.local
          ...
          .catch((error) => {
            ...
            sendResponse({
              success: false,
              error: "Failed to save photo to storage: " + (error.message || error),
            });
          });
      };
      reader.onerror = (error) => {
        ...
        sendResponse({
          success: false,
          error: "Failed to read photo data from blob: " + (error.message || error),
        });
      };
    ...
    .catch((error) => {
      ...
      sendResponse({
        success: false,
        error: error.message || "Failed to fetch photo due to an unknown reason.",
      });
    });
}
